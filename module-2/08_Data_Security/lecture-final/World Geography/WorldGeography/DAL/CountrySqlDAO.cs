        ...
        public IList<Country> GetCountries(string continent)
        {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    // This way is a more convenient way of doing the commented code below it
                    // Create the query and command object
                    string sql = "Select * from country where continent = @continentToSearchFor or @continentToSearchFor = ''";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.Parameters.AddWithValue("@continentToSearchFor", continent);

                    // 
                    // This way works
                    //
                    //SqlCommand cmd;
                    //if (continent.Length == 0)      // (continent == "")
                    //{
                    ...
                    //else
                    //{
                    //    string sql = "Select * from country where continent = @continentToSearchFor";
                    //    cmd = new SqlCommand(sql, conn);
                    //    cmd.Parameters.AddWithValue("@continentToSearchFor", continent);
                    //}
