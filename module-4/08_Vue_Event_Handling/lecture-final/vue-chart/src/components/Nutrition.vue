    ...
    <table style="text-align: right;">
      <thead>
        <tr>
          ...
          <!-- TODO01: add a button to add a new meal -->
          <th><input type="button" class="add" value="Add Meal" v-on:click="newMeal" /></th>
        </tr>
      </thead>
      <tbody>
        <!-- TODO02: add an index to the foreach so that we can pass it into Remove -->
        <tr v-for="(meal, index) in meals" v-bind:key="meal.id">
          ...
          <td>{{calories(meal)}}</td>
          <!-- TODO02: Add a link to remove the current meal -->
...
export default {
  ...
  computed: {
    ...
    // TODO01: Add a computed property to calculate the next id for a meal 
    nextMealId() {
      ...
      return this.meals[this.meals.length-1].id +1;
    }
  },
  ...
  methods: {
    ...
    /**
    TODO01: The newMeal() method adds a new, empty meal to the collection of meals
     */
    newMeal() {
      let meal = {
          id: this.nextMealId,
    ...
    /**
    TODO02: The removeMeal(index) method removes the meal at the specified index
     */
    removeMeal(index) {
