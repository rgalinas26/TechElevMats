<template>
    <div class="main">
        ...
        <div class="well-display">
            <div class="well">
                <!-- TODO 08: on click, set filter to 0, to find all ratings -->
                <span class="amount" v-on:click="filter = 0">{{ averageRating }}</span>
                Average Rating
            </div>

            <div class="well">
                <!-- TODO 08: on click, set filter to 1, to find 1-star ratings -->
                <span class="amount" v-on:click="filter = 1">{{ numberOfOneStarReviews }}</span>
                1 Star Review{{ numberOfOneStarReviews === 1 ? '' : 's' }}
            </div>

            <div class="well">
                <!-- TODO 08: on click, set filter to 2, to find 2-star ratings -->
                <span class="amount" v-on:click="filter = 2">{{ numberOfTwoStarReviews }}</span>
                2 Star Review{{ numberOfTwoStarReviews === 1 ? '' : 's' }}
            </div>

            <div class="well">
                <!-- TODO 08: on click, set filter to 3, to find 3-star ratings -->
                <span class="amount" v-on:click="filter = 3">{{ numberOfThreeStarReviews }}</span>
                3 Star Review{{ numberOfThreeStarReviews === 1 ? '' : 's' }}
            </div>

            <div class="well">
                <!-- TODO 08: on click, set filter to 4, to find 4-star ratings -->
                <span class="amount" v-on:click="filter = 4">{{ numberOfFourStarReviews }}</span>
                4 Star Review{{ numberOfFourStarReviews === 1 ? '' : 's' }}
            </div>

            <div class="well">
                <!-- TODO 08: on click, set filter to 5, to find 5-star ratings -->
                <span class="amount" v-on:click="filter = 5">{{ numberOfFiveStarReviews }}</span>
                5 Star Review{{ numberOfFiveStarReviews === 1 ? '' : 's' }}
            </div>
        </div>

        <!-- TODO 02: Set the showForm variable to TRUE. Show this button only of ShowForm is FALSE. -->
        <a href="#" v-on:click="showForm = true" v-if="showForm === false" >Show Form</a>

        <!-- TODO 03: Show this form only when showForm is TRUE.  Call addNewReview when the form is submitted. -->
        <form v-if="showForm === true" v-on:submit.prevent="addNewReview">
            ...
            <!-- TODO xx: Disable this button when isFormValid = FALSE -->
            <button>Submit</button>

            <!-- TODO 04: Call resetForm, which will blank out fields and set showForm to FALSE -->
            <button type="cancel" v-on:click.prevent="resetForm" >Cancel</button>
        </form>

        <!-- TODO 07: use filteredReviews instead of the reviews array -->
...
export default {
    ...
    data() {
        return {
            ...
            // TODO 01: add a variable showForm which will determine if the Add Review form will be shown
            showForm: false,

            // TODO: 01: add a newReview empty object
            newReview: {},

            // TODO 05: add a variable filter, which contains the number of stars to show (0 for all)
            filter: 0,
    ...
    computed: {
        ...
        // TODO 06: Create a filteredReviews computed property to return a filtered array by star rating
        filteredReviews() {
